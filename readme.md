# SeedCash Emulator
> Allows to execute [SeedCash](https://github.com/SeedCashOrg) air-gapped hardware wallet in your desktop (windows/linux/mac)

![](img/ubuntu.png)


## How it works
I made a new display driver that replaces the existing _SeedCash ST7789 1.3_ driver, making possible to execute the device in a desktop enviroment.

>Use it with the integrated buttons or with your keyboard ( Arrow Keys, Enter, 1,2,3 )

I have tried to make it as non-invasive as possible so that the emulator can be used with other versions of SeedCash with as little effort as possible.
...
**1-** Clone [SeedCash](https://github.com/SeedCashOrg/seedcash) repository
...
Download the contents from this respository [SeedCash emulator (.zip)](https://github.com/SeedCashOrg/seedcash-emulator.git) and merge the contents of the folder 

![](img/tree_content.png)

with the existing **seedcash** content you cloned in step 1. 
...
On the folder entered with the command `cd seedcash/src`, using command line (the command rsync below will not work on Windows. You will need to copy from `seedcash-emulator/seedcash` the folder `emulator`, and the replace the following files: `gui\renderer.py`, `hardware\camera.py`, `hardware\buttons.py`, `hardware\pivideostream.py`, `resources\icons\emulator_icon.png`)

```sh
git clone http://github.com/SeedCashOrg/seedcash-emulator.git
rsync -a seedcash-emulator/seedcash ./
```
...
```sh
pyinstaller --clean --add-data seedcash\\resources;seedcash\\resources main.py
```
If you found an error **unable to execute command pyinstaller** try using the following command

```sh
python3 -m pyinstaller --clean --add-data seedcash\\resources;seedcash\\resources main.py
```
...
These are some notes that could help you installing SeedCash Emulator on Tails OS
...
- This emulator is made for testing SeedCash source code on your desktop.   
- You can modify or contribute to seedcash without the need to upload the code to a  Raspberry Pi Zero each time 
